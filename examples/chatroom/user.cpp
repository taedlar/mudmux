...
void User::prompt() {
    ...
    if (comm_slot < 0)
        return;
    comm_buffered_write (comm_slot, "> ", 2); // display prompt for user input
}
    ...
    if (inbound_handler) {
        ...
        if (message.front() == '/') {
            ...
            if (!Command::find_and_execute(message.substr(1), shared_from_this())) { // remove the leading '/' before searching for the command
                addMessage("Unknown slash command: " + message + "\n");
            }
        } else {
...
void User::receiveUsername(const std::string& name) {
    ...
    if (name.length() < 3 || name.length() > 16) {
        addMessage("Username must be between 3 and 16 characters. Please enter a valid username: ");
    ...
    addMessage(fmt::format("Welcome, {}!\n", username));
    addMessage("You are now logged in. Type your messages to chat with others.\n");
    addMessage("You can also use slash commands like /help, /quit, etc. to interact with the chatroom.\n");
}
